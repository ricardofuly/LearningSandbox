...
#include "SLoadingScreenSettings.h"
...
void USLoadingScreenManager::Initialize(FSubsystemCollectionBase& Collection)
{
...
void USLoadingScreenManager::Deinitialize()
{
	StopBlockingInput();
...
bool USLoadingScreenManager::ShouldCreateSubsystem(UObject* Outer) const
{
...
void USLoadingScreenManager::Tick(float DeltaTime)
{
	UpdateLoadingScreen();
	...
	{
		if (const FWorldContext* Context = GetGameInstance()->GetWorldContext())
		{
			float AsyncProgress = GetAsyncLoadPercentage(*Context->LastURL.Map);
			if (AsyncProgress >- 0.f)
			{
				CurrentLoadingScreenProgress = AsyncProgress / 100.f;
			}
		}
	}
}

bool ISLoadingProcessInterface::ShouldShowLoadingScreen(UObject* TestObject, FString& OutReason)
{
	if (TestObject != nullptr)
	{
		if (ISLoadingProcessInterface* LoadObserver = Cast<ISLoadingProcessInterface>(TestObject))
		{
...
ETickableTickType USLoadingScreenManager::GetTickableTickType() const
{
	if (IsTemplate())
	{
...
bool USLoadingScreenManager::IsTickable() const
{
...
TStatId USLoadingScreenManager::GetStatId() const
{
	RETURN_QUICK_DECLARE_CYCLE_STAT(ULoadingScreenManager, STATGROUP_Tickables);
}

UWorld* USLoadingScreenManager::GetTickableGameObjectWorld() const
{
	return GetGameInstance()->GetWorld();
}

float USLoadingScreenManager::GetLoadingProgress() const
{
	return CurrentLoadingScreenProgress;
}

void USLoadingScreenManager::RegisterLoadingProcessor(TScriptInterface<ISLoadingProcessInterface> Interface)
{
	ExternalLoadingProcessors.Add(Interface.GetObject());
}

void USLoadingScreenManager::UnregisterLoadingProcessor(TScriptInterface<ISLoadingProcessInterface> Interface)
{
	ExternalLoadingProcessors.Remove(Interface.GetObject());
}

void USLoadingScreenManager::HandlePreLoadMap(const FWorldContext& WorldContext, const FString& MapName)
{
...
void USLoadingScreenManager::HandlePostLoadMap(UWorld* World)
{
...
void USLoadingScreenManager::UpdateLoadingScreen()
{
	bool bLogLoadingScreenStatus = LoadingScreenCVars::LogLoadingScreenReasonEveryFrame;

	if (ShouldShowLoadingScreen())
	{
		const USLoadingScreenSettings* Settings = GetDefault<USLoadingScreenSettings>();
...
bool USLoadingScreenManager::CheckForAnyNeedToShowLoadingScreen()
{
	...
	if (ISLoadingProcessInterface::ShouldShowLoadingScreen(GameState, /*out*/ DebugReasonForShowingOrHidingLoadingScreen))
	{
		return true;
	}
	...
	{
		if (ISLoadingProcessInterface::ShouldShowLoadingScreen(TestComponent, /*out*/ DebugReasonForShowingOrHidingLoadingScreen))
		{
			return true;
		}
	}
	...
	// streaming in.
	for (const TWeakInterfacePtr<ISLoadingProcessInterface>& Processor : ExternalLoadingProcessors)
	{
		if (ISLoadingProcessInterface::ShouldShowLoadingScreen(Processor.GetObject(), /*out*/ DebugReasonForShowingOrHidingLoadingScreen))
		{
			return true;
		}
	}
	...
	for (ULocalPlayer* LP : LocalGameInstance->GetLocalPlayers())
	{
		if (LP != nullptr)
		{
			if (APlayerController* PC = LP->PlayerController)
			{
				...
				if (ISLoadingProcessInterface::ShouldShowLoadingScreen(PC, /*out*/ DebugReasonForShowingOrHidingLoadingScreen))
				{
					return true;
				}
...
bool USLoadingScreenManager::ShouldShowLoadingScreen()
{
	const USLoadingScreenSettings* Settings = GetDefault<USLoadingScreenSettings>();
...
bool USLoadingScreenManager::IsShowingInitialLoadingScreen() const
{
...
void USLoadingScreenManager::ShowLoadingScreen()
{
...
void USLoadingScreenManager::HideLoadingScreen()
{
	if (!bCurrentlyShowingLoadingScreen)
	{
		return;
...
void USLoadingScreenManager::RemoveWidgetFromViewport()
{
	UGameInstance* LocalGameInstance = GetGameInstance();
...
void USLoadingScreenManager::StartBlockingInput()
{
...
void USLoadingScreenManager::StopBlockingInput()
{
	if (InputPreProcessor.IsValid())
	{
...
void USLoadingScreenManager::ChangePerformanceSettings(bool bEnabingLoadingScreen)
{
	UGameInstance* LocalGameInstance = GetGameInstance();
	UGameViewportClient* GameViewportClient = LocalGameInstance->GetGameViewportClient();
