...
#include "WaitCooldownChangedAsync.h"

UWaitCooldownChangedAsync* UWaitCooldownChangedAsync::ListenForCooldownChange(
	UAbilitySystemComponent* AbilitySystemComponent, FGameplayTagContainer InCooldownTags, bool InUseServerCooldown)
{
	UWaitCooldownChangedAsync* ListenForCooldownChange = NewObject<UWaitCooldownChangedAsync>();
	...
	AbilitySystemComponent->OnActiveGameplayEffectAddedDelegateToSelf.AddUObject(ListenForCooldownChange, &UWaitCooldownChangedAsync::OnActiveGameplayEffectAddedCallback);
	...
	{
		AbilitySystemComponent->RegisterGameplayTagEvent(CooldownTag, EGameplayTagEventType::NewOrRemoved).AddUObject(ListenForCooldownChange, &UWaitCooldownChangedAsync::CooldownTagChanged);
	}

	return ListenForCooldownChange;
}

void UWaitCooldownChangedAsync::EndTask()
{
	if (IsValid(ASC))
	{
...
void UWaitCooldownChangedAsync::OnActiveGameplayEffectAddedCallback(UAbilitySystemComponent* Target,
...
void UWaitCooldownChangedAsync::CooldownTagChanged(const FGameplayTag CooldownTag, int32 NewCount)
{
	if (NewCount == 0)
	{
...
bool UWaitCooldownChangedAsync::GetCooldownRemainingForTag(FGameplayTagContainer InCooldownTags, float& TimeRemaining,
